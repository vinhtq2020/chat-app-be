...
func (r *FriendRepository) Load(ctx context.Context, userId1 string, userId2 string, relationType domain.RelationType) (*domain.Relation, error) {
	...
	err := sql.Query(r.db, stmt, &res, r.logger, userId1, userId2, relationType)
	if err != nil {
...
func (r *FriendRepository) Upsert(ctx context.Context, friend domain.Relation) (int64, error) {
	...
	qr := "insert into %s(user_id1, user_id2, status, created_at, created_by, updated_at, updated_by) values (%s, %s, %s, %s, %s, %s, %s) on conflict(user_id1, user_id2) do update set status = %s, updated_at = %s, updated_by = %s"
	stmt := fmt.Sprintf(qr, r.table, r.buildParam(1), r.buildParam(2), r.buildParam(3), r.buildParam(4),
		r.buildParam(5), r.buildParam(6), r.buildParam(7),
		r.buildParam(3), r.buildParam(6), r.buildParam(7))
	res, err := sql.Exec(db, stmt, r.logger, friend.UserId1, friend.UserId2, friend.Status, friend.CreatedAt, friend.CreatedBy, friend.UpdatedAt, friend.UpdatedBy, friend.Status, friend.UpdatedAt, friend.UpdatedBy)
	if err != nil {
		r.logger.LogError(err.Error(), nil)
		return -1, err
	}
